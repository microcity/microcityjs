...
模块存在于离散事件仿真的单线程中，分为“实体（Entity）”和“调度器（Planner）“两种，并将数据、文档、程序包装成zip文件供系统调用。”实体“占据空间，存在有限的状态和行为模式并可以接受有限的指令，有一定的自主决策能力，”调度器“不占据空间，纯算法，由事件触发（实体事件、其他调度器事件或定时事件），获取实体状态并能向实体发送指令。
